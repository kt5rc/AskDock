...
export default function HomePage() {
  ...
  const handleCreate = async (draft: { title: string; body: string; category: string }) => {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          {user && <p className="text-sm text-muted-foreground">Welcome back, {user.display_name}</p>}
        </div>
      ...
      <Card className="card-sheen">
        <CardContent className="space-y-4">
          <div className="text-sm font-semibold">Post a new question</div>
          <MemoEditor submitLabel="Post question" onSubmit={handleCreate} disabled={!user} />
        </CardContent>
      </Card>
      ...
      {loading && <div className="text-sm text-muted-foreground">Loading memos...</div>}
