...
export default function HomeClient({
  ...
  const { data, error, isLoading, mutate } = useSWR(
    `/api/memos?${queryString}`,
    fetcher,
    { revalidateOnFocus: false, keepPreviousData: true, fallbackData: initialData, revalidateOnMount: false }
  );
  ...
  const { data: countsData, mutate: mutateCounts } = useSWR(
    `/api/memos/counts?${countQuery}`,
    fetcher,
    { revalidateOnFocus: false, dedupingInterval: 5000, fallbackData: initialData.counts, revalidateOnMount: false }
  );
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="min-h-[1.25rem] text-sm text-muted-foreground">
            {initialUser ? `Welcome back, ${initialUser.display_name}` : "\u00A0"}
          </p>
        </div>
